...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    :bind (("C-c n l" . org-roam-buffer-toggle)
	   ...
	   ;; the following are duplicates from zetteldeft
	   ("C-c n s" . zetteldeft-search-at-point)
	   ("C-c n d" . zetteldeft-deft-new-search)
	   ("C-c n r" . deft-refresh)
	   ("C-c n h" . zetteldeft-go-home)
	   ("C-c n x" . zetteldeft-count-words)
	   :map org-mode-map)
    :config
    (org-roam-setup))
#+end_src

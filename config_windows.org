...
** note on encoding
if there are any encoding issues with øæå, the try to replace them like
this: æ -> æ check encoding on the lower left side of the document.
...
(setq org-capture-templates
  '(("t" "TODO" entry (file ORG_CAPTURE)
    ...
    ("d" "Diary" entry (file+datetree ORG_DIARY)
     "* Dagen i går%?")
    ("m" "Mætelogg" entry (file+datetree ORG_MOTELOGG)
     "* %? \nInnkalling: \nAgenda: \nNotater:")
...
(global-set-key (kbd "C-c ø") 'unhighlight-all)
(global-set-key (kbd "C-c æ") 'highlight-symbol-at-point)
(global-set-key (kbd "C-c å") 'org-insert-image)
(global-set-key (kbd "C-æ") 'make-frame-command)
(global-set-key (kbd "C-ø") 'delete-frame)
(global-set-key (kbd "C-å") 'toggle-frame-solidity)
(global-set-key (kbd "M-æ") 'kmacro-start-macro-or-insert-counter)
(global-set-key (kbd "M-ø") 'kmacro-end-or-call-macro)

...
#+begin_src emacs-lisp
  ...
  (setq my-init-modules '("Dashboard" "Keybinding" "Experimental" "Emojis" "Pandoc" "Themes" "Python" "R" "Agenda" "Zetteldeft" "Teos" "Packages" "Constants" "Elfeed" "Windows_specific"))
  ...
  ;; loading windows specific code
  (when (eq system-type 'windows-nt)
   (ignore-errors (org-babel-load-file "~/emacs-config/mod/windows_specific.org")))
  ;; experimental
...
#+begin_src emacs-lisp
  ...
  (when (member "Zetteldeft" loaded-modules)
    (deft))
  (when (string= (system-name) SYSTEM_HOME)
    (toggle-frame-solidity)) ;; if the starup begins with split screen
  (dashboard-open)
#+end_src

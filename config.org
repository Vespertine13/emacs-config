...
#+begin_src emacs-lisp
  ...
  (defun load-module-and-measure-time (module file)
	...
	(if (> elapsed-time 0)
      ...
      (setq my-init-modules '("Keybinding" "Experimental" "Functions" "EMMS" "Abbrev" "Mastodon" "Elfeed" "Emojis" "Pandoc" "Themes" "Python" "R" "Company" "Agenda" "Zettelkasten" "Teos" "Packages" "Constants"))
      ;; Make a list of loaded modules
      (setq loaded-modules ())
      ;; Loading modules
      (load-module-and-measure-time "Constants" (concat FOLDER_CLOUD "config/constants.org"))
      (load-module-and-measure-time "Packages" "~/emacs-config/mod/packages.org")
      (load-module-and-measure-time "Teos" (concat FOLDER_CLOUD "config/teos.org"))
      ...
      (load-module-and-measure-time "Emojis" (concat FOLDER_CLOUD "config/emojis.org"))
      (load-module-and-measure-time "Elfeed" (concat FOLDER_CLOUD "config/elfeed.org"))
      ...
      (load-module-and-measure-time "Experimental" (concat FOLDER_CLOUD "config/experimental.org"))
...
#+begin_src emacs-lisp
  ...
  ;; Set the footer
  (when (member "Experimental" loaded-modules)
    (setq dashboard-footer-messages (list (concat "" (random-item my-emacs-tips)))))
...
#+begin_src emacs-lisp
  ...
  (defun dashboard-startup-time (list-size)
    (insert (concat " Startup at: " (current-time-string))))

  (defun dashboard-line (list-size)
    (insert "------------------------------------------------------------------------------------------"))

  (add-to-list 'dashboard-item-generators '(line . dashboard-line))
  (add-to-list 'dashboard-item-generators '(custom-startup-time . dashboard-startup-time))
  ...
  (setq dashboard-items '((custom-startup-time) (custom-loaded-modules) (custom-links)))
#+end_src
* Encryption
#+begin_src emacs-lisp

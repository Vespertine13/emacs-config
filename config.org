...
#+begin_src emacs-lisp
  ...
  (defun theme-ef (name)
    (disable-all-themes)
    (load-theme (intern (format "ef-%s" name)) t)
    (set-frame-font "Inconsolata 16" nil t)
    (message "theme ef-%s" name))

  (defun set-season-theme ()
    (let* ((current-month (format-time-string "%m" (current-time)))
	   (winter '("12" "01" "02"))
	   (spring '("03" "04" "05"))
	   (summer '("06" "07" "08"))
	   (autumn '("09" "10" "11")))
      (cond ((member current-month winter) (theme-ef "winter"))
	    ((member current-month spring) (theme-ef "spring"))
	    ((member current-month summer) (theme-ef "summer"))
	    ((member current-month autumn) (theme-ef "autumn")
	     ))))
...
#+begin_src emacs-lisp
  ...
  (defun write-yesterday-date ()
    "Writes yesterday's date at the current position"
    (let ((yesterday (time-subtract (current-time) (seconds-to-time 86400))))
      (insert (format-time-string "%d-%m-%Y" yesterday))))
...
#+begin_src emacs-lisp
  ...
  (defun workout-diary ()
    ...
    (end-of-buffer)
      ...
      (save-buffer)
      (save-text-as-file ",bench_120,dips_120,chinups,pullups,squat_120,wheel\n,NA,NA,NA,NA,NA,NA" WORKOUT_LOG))

...
#+begin_src emacs-lisp
  (defun coding-playlist()
    "runs the coding playlist in mpv asynchronously."
    (interactive)
    (when (string= (system-name) HOME_SYSTEM)(async-shell-command PATH_CODING_PLAYLIST_HOME))
    (when (string= (system-name) WORK_SYSTEM)(async-shell-command PATH_CODING_PLAYLIST_WORK))
    )
#+end_src
